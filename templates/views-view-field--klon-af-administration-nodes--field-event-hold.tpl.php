...
if(isset($row->field_field_event_hold))
{
    foreach($row->field_field_event_hold as $i => $item)
    {
        ...
        if($i == 0)
        {
            echo "<a target='_blank' href='/field-collection/field-event-hold/".$entity->item_id."/edit'><div style='display:inline-block;color:".$bg_color."';'>".$entity->field_hold_navn['und'][0]['value']."</div></a>";
        }
        else
        {
             echo ", <a target='_blank' href='/field-collection/field-event-hold/".$entity->item_id."/edit'><div style='display:inline-block;color:".$bg_color.";'>".$entity->field_hold_navn['und'][0]['value']."</div></a>";
